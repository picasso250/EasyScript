...
*   **强类型运行时 (Strongly Typed Runtime)**：EasyScript 奉行强类型原则，运行时不会在不同类型之间进行隐式转换。
    *   **内置函数错误处理**：对于如 `number()` 这样的类型转换内置函数，当转换失败（例如 `number("abc")`）或遇到不支持的类型（例如 `number([1])`）时，它们会返回 **`nil`**，而不是抛出运行时错误。这使得脚本能够优雅地处理可预见的错误情况。只有当函数用法本身存在问题（如参数数量不正确）时，才会抛出运行时错误。
