...
*   **统一的真值规则 (Unified Truthiness Rules)**: EasyScript 的条件判断（如 `if` 表达式）和 `bool()` 类型转换函数遵循一套统一的、类似 Python 的真值规则。
...
*   **强类型运行时 (Strongly Typed Runtime)**：EasyScript 奉行强类型原则，运行时不会在不同类型之间进行隐式转换。
    *   **内置函数错误处理**：对于如 `number()` 这样的类型转换内置函数，当转换失败（例如 `number("abc")`）或遇到不支持的类型时，它们会返回 **`nil`**，而不是抛出运行时错误。只有当函数用法本身存在问题（如参数数量不正确）时，才会抛出运行时错误。
    *   **`+` 运算符的哲学**：`+` 运算符用于“连接”或“相加”两个**相同类型**的容器或值。
        *   **数字 + 数字**：执行数学加法。
        *   **字符串 + 字符串**：执行字符串拼接。
