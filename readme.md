...
| **赋值** | `myVar = 10 * (a + b);` | 支持基础赋值。 |
| **条件分支** | `if a > 10 { result = 1; } else { result = 0; }` | 条件外无括号，强制使用代码块。 |
| **循环 (While-Style)** | `for i < 10 { i = i + 1; }` | 用于无限循环或基于条件的循环。 |
| **循环 (Collection-Style)** | `for item in range(0, 10) { sum = sum + item; }` | 遍历列表等集合（例如内置 `range()` 函数的结果）。 |

## 总结

**EasyScript** 证明了**极简**与**功能强大**可以完美共存。通过精心设计的 BNF 结构，以及对函数和精简循环的引入，我们创造了一个易于实现、易于学习、且完全具备执行任何算法能力的脚本语言。它是学习编程语言设计和算法逻辑的理想选择。