...
*   **分号 (Semicolon): 分隔符与副作用标记**
在 EasyScript 中，分号 `;` 扮演着双重重要角色：
1.  **表达式分隔符**：在代码块或程序顶层，分号 `;` 用于分隔多个顺序执行的表达式。例如，`expr1; expr2; expr3` 会依次计算三个表达式。两个独立的表达式之间必须使用分号分隔。
2.  **副作用标记**：当一个表达式的主要目的是其副作用（如赋值、打印）而非其返回值时，可以在其后加上分号 `;`。这会使该表达式（而非整个序列）的最终返回值为 **`nil`**，清晰地向阅读者和编译器传达“我们不关心这个表达式的结果”。
...
*   **强类型运行时 (Strongly Typed Runtime)**：EasyScript 不会在不同类型之间进行隐式转换。
    ...
    *   **Map 键类型**：Map 的键必须是基本值类型，包括 `String`、`Number` 和 `Boolean`。这些类型是可哈希和可比较的，能够提供稳定且可预测的键查找行为。不允许使用列表、映射、函数等复杂类型作为键，以避免哈希不确定性和可变性问题。
